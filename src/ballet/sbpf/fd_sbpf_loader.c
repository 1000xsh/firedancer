...
_fd_sbpf_elf_peek( fd_sbpf_elf_info_t * info,
  ...
  *info = (fd_sbpf_elf_info_t) {
  ...
  /* Section headers */
  if( FD_UNLIKELY( (err=fd_sbpf_load_shdrs( info, elf,  elf_sz, deploy_mode ))!=0 ) )
    return err;
...
fd_sbpf_elf_peek( fd_sbpf_elf_info_t * info,
                  ...
                  ulong                elf_sz,
                  bool                 deploy_mode ) {
  return (_fd_sbpf_elf_peek( info, bin, elf_sz, deploy_mode ) == 0) ? info : NULL;
}
